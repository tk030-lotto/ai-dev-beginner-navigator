...
export const gitConceptPlugin: ProblemPlugin = {
  ...
  knowledge: {
    ...
    steps: [
      ...
      {
        step: 4,
        title: '遠隔地から最新コードを取得する: git pull',
        detail: 'チームメンバーや別のPCからGitHubにプッシュされた最新のコミットを自分のPCに取り込むのが pull です。湖で言えば「雲側の最新版を販尊する」イメージです。',
    ...
    cautions: [
      ...
      '作業中はgit statusで現在の状態を頻繁に確認する習慣をつけるとミスマッチやコミット漏れを防げます。',
    ],
  },
...
2. 今自分のGitがどんな状態にあるか（セーブ漏れがないか）を確認するための最も安全なコマンド（git status等）と、その見方を教えてください。`;
    ...
    {
      targetAi: 'Gemini',
      title: 'Gitワークフローの全体像とメンタルモデルの拡張',
      generate: () => {
        return `Gitの初心者です。「コミット、プッシュ、プル」の3操作は業務上こなせるようになりましたが、「ブランチを切り替える理由」や「ステージングとコミットの間に何があるのか」がまだ曖昧です。

以下の点を日常の創作活動・ゲームセーブなど、初心者が直感的に理解できる例えに置き換えて解説してください。

1. ステージングエリア（git add）とリポジトリ（git commit）の差分
2. ブランチを使う理由（なぜ1本のメインブランチだけではダメなのか）
3. git pullが必要なタイミングとその危険性（コンフリクト）`;
      },
    },
  ],
};
